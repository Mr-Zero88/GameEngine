...
//   {
//     vr::VR_Shutdown();
//     printf("Unable to get render model interface: %s", vr::VR_GetVRInitErrorAsEnglishDescription(error));
//     return 1;
//   }
...
//   {
//     vr::VR_Shutdown();
//     printf("Could not determine OpenVR Vulkan instance extensions.\n");
//     return 1;
//   }
...
public:
  ...
  void installManifest(std::string applicationKey, bool cleaninstall = false);
  void removeManifest(std::string applicationKey);
  bool BInit();
  bool BInitVulkan();
...
CMainApplication::CMainApplication(int argc, char *argv[])
    : m_pCompanionWindow(NULL), m_nCompanionWindowWidth(640), m_nCompanionWindowHeight(320), m_pHMD(NULL), m_pRenderModels(NULL), m_bDebugVulkan(false), m_bVerbose(false), m_bPerf(false), m_bVblank(false), m_nMSAASampleCount(4), m_flSuperSampleScale(1.0f), m_iTrackedControllerCount(0), m_iTrackedControllerCount_Last(-1), m_iValidPoseCount(0), m_iValidPoseCount_Last(-1), m_iSceneVolumeInit(20), m_strPoseClasses(""), m_bShowCubes(true), m_pInstance(VK_NULL_HANDLE), m_pDevice(VK_NULL_HANDLE), m_pPhysicalDevice(VK_NULL_HANDLE), m_pQueue(VK_NULL_HANDLE), m_pSurface(VK_NULL_HANDLE), m_pSwapchain(VK_NULL_HANDLE), m_pDebugReportCallback(VK_NULL_HANDLE), m_pCommandPool(VK_NULL_HANDLE), m_pDescriptorPool(VK_NULL_HANDLE), m_nSwapQueueImageCount(0), m_nFrameIndex(0), m_nCurrentSwapchainImage(0), m_pSceneVertexBuffer(VK_NULL_HANDLE), m_pSceneVertexBufferMemory(VK_NULL_HANDLE), m_pSceneVertexBufferView(VK_NULL_HANDLE), m_pSceneImage(VK_NULL_HANDLE), m_pSceneImageMemory(VK_NULL_HANDLE), m_pSceneImageView(VK_NULL_HANDLE), m_pSceneStagingBuffer(VK_NULL_HANDLE), m_pSceneStagingBufferMemory(VK_NULL_HANDLE), m_pSceneSampler(VK_NULL_HANDLE), m_pDescriptorSetLayout(VK_NULL_HANDLE), m_pPipelineLayout(VK_NULL_HANDLE), m_pPipelineCache(VK_NULL_HANDLE), m_pCompanionWindowVertexBuffer(VK_NULL_HANDLE), m_pCompanionWindowVertexBufferMemory(VK_NULL_HANDLE), m_pCompanionWindowIndexBuffer(VK_NULL_HANDLE), m_pCompanionWindowIndexBufferMemory(VK_NULL_HANDLE), m_pControllerAxesVertexBuffer(VK_NULL_HANDLE), m_pControllerAxesVertexBufferMemory(VK_NULL_HANDLE)
{
...
void CMainApplication::installManifest(std::string applicationKey, bool cleaninstall)
{
  auto manifestPath = fs::absolute("manifest.vrmanifest");
  if (fs::exists(manifestPath))
  {
    ...
    if (vr::VRApplications()->IsApplicationInstalled(applicationKey.c_str()))
    {
      if (cleaninstall)
      {
        ...
        {
          auto workingDirectory = fs::path(std::string(buffer));
          auto oldManifestPath = fs::absolute(workingDirectory.append("manifest.vrmanifest"));
          if (oldManifestPath.compare(manifestPath) != 0)
          {
            vr::VRApplications()->RemoveApplicationManifest(oldManifestPath.string().c_str());
          }
          else
          {
    ...
    {
      throw std::runtime_error("Application is not Installed");
    }
    // char buffer[1024];
    // vr::VRApplications()->IdentifyApplication(0, buffer);
    // std::cout << ("App: " + std::string(buffer)).c_str();
    // SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "VRApplications", std::string(buffer).c_str(), NULL);
    // throw std::runtime_error(std::string(buffer));

    auto apperror = vr::VRApplications()->AddApplicationManifest(manifestPath.string().c_str());
    if (apperror != vr::VRApplicationError_None)
    {
    ...
    /*
      ...
      if (apperror != vr::VRApplicationError_None) {
        auto vrerror = std::string(vr::VRApplications()->GetApplicationsErrorNameFromEnum(apperror));
        replace(vrerror, "_", ": ");
        throw std::runtime_error(std::string("Could not set auto start: ") + vrerror);
      }
    }*/
  }
  else
  {
    throw std::runtime_error("Could not find application manifest: " + manifestPath.string());
  }
}

void CMainApplication::removeManifest(std::string applicationKey)
{
  auto manifestPath = fs::absolute("manifest.vrmanifest");
  if (fs::exists(manifestPath))
  {
    if (vr::VRApplications()->IsApplicationInstalled(applicationKey.c_str()))
    {
      vr::VRApplications()->RemoveApplicationManifest(manifestPath.string().c_str());
    }
  }
  else
  {
...
VulkanRenderModel::VulkanRenderModel(const std::string &sRenderModelName)
    : m_sModelName(sRenderModelName), m_pDevice(VK_NULL_HANDLE), m_pVertexBuffer(VK_NULL_HANDLE), m_pVertexBufferMemory(VK_NULL_HANDLE), m_pIndexBuffer(VK_NULL_HANDLE), m_pIndexBufferMemory(VK_NULL_HANDLE), m_pImage(VK_NULL_HANDLE), m_pImageMemory(VK_NULL_HANDLE), m_pImageView(VK_NULL_HANDLE), m_pImageStagingBuffer(VK_NULL_HANDLE), m_pImageStagingBufferMemory(VK_NULL_HANDLE), m_pSampler(VK_NULL_HANDLE)
{
